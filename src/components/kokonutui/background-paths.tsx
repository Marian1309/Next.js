...
const AnimatedTitle = memo(({ title }: { title: string }) => {
  return (
    <motion.h1
      className="mb-8 bg-gradient-to-r from-neutral-800/90 to-neutral-600/90 bg-clip-text text-3xl font-bold tracking-tighter text-transparent sm:text-5xl md:text-5xl dark:from-white/90 dark:to-white/70"
...
const BackgroundPaths = ({ title = 'Background Paths', children }: Properties) => {
  return (
    <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-white dark:bg-neutral-950">
      ...
      <div className="relative z-10 container mx-auto px-4 text-center md:px-6">
        ...
        >
          {children ? children : <AnimatedTitle title={title} />}
        </motion.div>
      </div>
    </div>
  );
